...
private final MessageService messageService;
    ...
    @Operation(
            ...
            description = """
                    ...
                    a placeholder preview. TransactWriteItems is limited to 100 items, so the first \
                    chunk is metadata plus up to 99 participants and later chunks hold up to 100 \
                    participants. A crash between sequential chunks leaves lifecycleState CREATING \
                    until a matching clientRequestId retry repairs missing members. Supplying \
                    clientRequestId lets callers replay the same creation and repair its inbox \
                    projections.""")
    ...
    @ApiResponse(responseCode = "400", description = "Missing X-User-Id, invalid type, bad participant count, invalid title, or reused clientRequestId with different content",
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
    ...
    @ApiResponse(responseCode = "400", description = "Malformed conversationId, missing X-User-Id, sender not a participant, or reused clientRequestId with different content",
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = ErrorResponse.class)))
