 ...
 *
 * <p>Domain and DynamoDB classification live in {@code ExceptionMapper}. Each {@code @ExceptionHandler}
 * method stays on this advice class so OpenAPI status mappings remain here, and delegates to that
 * registry. Async MVC controllers return {@code CompletableFuture}. Failures thrown inside
 * {@code thenApply} or {@code thenCompose} chains arrive wrapped in {@link CompletionException}.
 * {@link #handleCompletionException(CompletionException)} unwraps the cause and looks it up in the
 * mapper before the generic {@link #handleUnexpectedException(Exception)} fallback runs, so wrapped
 * dependency faults keep their distinct, routable error code.
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleMissingActor(MissingActorException ex) {
        return exceptionMapper.map(ex).orElseThrow();
    }
     ...
     * @return error envelope mapped by {@code ExceptionMapper}
     */
     ...
     * on async MVC controllers. The cause is looked up in {@code ExceptionMapper}. Unmapped causes
    ...
    public ResponseEntity<ErrorResponse> handleCompletionException(CompletionException ex) {
        ...
        return exceptionMapper.map(cause).orElseGet(() -> handleUnexpectedException(ex));
    }
     ...
     * {@code INTERNAL_ERROR}. The generic path includes the request reference id in {@code message}
     * so a client can correlate the 500 with the {@code ERROR} log line. Mapped domain and DynamoDB
     * responses keep their existing message shape.
     *
    ...
    public ResponseEntity<ErrorResponse> handleUnexpectedException(Exception ex) {
        ...
        if (ddbCause != null) {
            return exceptionMapper.map(ddbCause).orElseThrow();
        }
        ...
        String requestId = RequestReference.current();
        logger.error("Unexpected server error [requestId={}]", requestId, ex);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ErrorResponse("INTERNAL_ERROR",
                        "An unexpected error occurred. Reference: " + requestId, Instant.now()));
    }
