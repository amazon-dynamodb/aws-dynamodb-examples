...
private final LikeMapper likeMapper;
     ...
     * failure. The cancellation reasons are ordered by {@link LikeTransactItemOrder}: a failed guard
     * on {@link LikeTransactItemOrder#LIKE_PUT} means a duplicate like
     * ({@link AlreadyLikedException}), while a failed guard on
     * {@link LikeTransactItemOrder#COUNTER_UPDATE} means the post disappeared between the read and
     * the write ({@link PostNotFoundException}).
     *
    ...
    private CompletableFuture<Void> classifyResult(Throwable error, String likerId, String postId) {
        ...
        if (cause instanceof TransactionCanceledException canceled && canceled.hasCancellationReasons()) {
            ...
            if (guardFailedAt(reasons, LikeTransactItemOrder.LIKE_PUT.index())) {
            ...
            if (guardFailedAt(reasons, LikeTransactItemOrder.COUNTER_UPDATE.index())) {
