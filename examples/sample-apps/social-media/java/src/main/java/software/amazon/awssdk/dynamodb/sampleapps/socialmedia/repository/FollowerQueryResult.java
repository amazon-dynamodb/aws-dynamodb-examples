 ...
 * and later followers are not read. An empty incomplete result means the in-process enumeration
 * rate limit skipped the Query.
 *
...
public record FollowerQueryResult(List<FollowerEdge> followers, boolean complete, int cap) {
    ...
    /**
     * Logs one {@code WARN} when this result is truncated by the row cap, with the followee id and
     * counts. An empty incomplete result is silent here because the enumeration limiter already logged
     * the skipped Query.
     *
