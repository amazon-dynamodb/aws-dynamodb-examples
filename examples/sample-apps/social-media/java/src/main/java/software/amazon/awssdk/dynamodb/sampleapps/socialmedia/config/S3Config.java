...
@Value("${s3.path-style:false}")
     ...
     *
     * <p>The client uses {@link DynamoDbConfig#API_CALL_ATTEMPT_TIMEOUT} and
     * {@link DynamoDbConfig#API_CALL_TIMEOUT} so a stalled {@code HEAD} is bounded the same way as
     * DynamoDB calls. Presigning does no network I/O and does not use this override.
     *
    ...
    /**
     * Call timeouts shared with {@link DynamoDbConfig} so S3 {@code HEAD} is bounded the same way
     * as DynamoDB.
     *
     * @return override configuration with the DynamoDB attempt and call timeouts
     */
    ClientOverrideConfiguration s3ClientOverrideConfiguration() {
        return ClientOverrideConfiguration.builder()
                .apiCallAttemptTimeout(DynamoDbConfig.API_CALL_ATTEMPT_TIMEOUT)
                .apiCallTimeout(DynamoDbConfig.API_CALL_TIMEOUT)
                .build();
    }
