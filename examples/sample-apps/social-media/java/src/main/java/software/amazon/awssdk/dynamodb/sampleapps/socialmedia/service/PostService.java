 ...
 * {@code DynamoDbStreamConsumer} materializes the same rows. Timeline visibility may then take at
 * least {@code dynamodb.async-minimum-visible-delay} while that consumer catches up. This service
 * does not wait for that delay on the request path.
 *
 ...
 *
 * <p>When {@code clientRequestId} is supplied, the service derives a stable post id and stores a
 * content fingerprint on the source row. A matching retry returns that post and re-runs synchronous
 * fan-out to repair an interrupted publication. A retry that reuses the id with different content is
 * rejected.
 *
...
public class PostService {
    ...
    public static final int DEFAULT_RESTRICTED_VIEWERS_MAX = 256;
...
private final StoryTtl storyTtl;
     ...
     * @param storyTtl              accepted {@code dynamodb.story-ttl-seconds} value
     */
    public PostService(UserGraphRepository userGraphRepository,
                       ...
                       @Value("${dynamodb.post-restricted-viewers-max:" + DEFAULT_RESTRICTED_VIEWERS_MAX + "}")
                       int restrictedViewersMax,
                       StoryTtl storyTtl) {
     ...
     * @throws ValidationException        when {@code clientRequestId} is reused with different content
     */
    public CompletableFuture<PublishPostResponse> publish(String actorUserId, PublishPostRequest request) {
        ...
        Long expiresAt = story ? now.getEpochSecond() + storyTtl.value() : null;
     ...
     * In {@code ASYNC} mode the request stops here and {@code DynamoDbStreamConsumer} performs the
     * same fan-out from the {@code POST_META} insert. Timeline visibility may take at least
     * {@code dynamodb.async-minimum-visible-delay}. Both modes produce identical timelines.
     */
