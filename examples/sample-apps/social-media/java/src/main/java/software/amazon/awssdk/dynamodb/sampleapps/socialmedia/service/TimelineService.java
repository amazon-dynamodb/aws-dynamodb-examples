 ...
 * read fails with {@link ValidationException}. The opaque {@code nextToken} is route-specific and
 * bound to the path user: a token minted for another list route or another user is rejected
 * downstream.
 * A token whose encoded depth is at or above {@code dynamodb.max-pages-allowed} is rejected with
 * {@link InvalidPaginationTokenException}. DynamoDB and S3 I/O stay async end-to-end.
 */
@Service
