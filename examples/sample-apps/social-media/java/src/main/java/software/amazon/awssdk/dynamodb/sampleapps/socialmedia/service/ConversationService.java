 ...
 * metadata plus 99 participants spans more than one sequential transaction. Inbox seeding uses
 * {@code dynamodb.inbox-fanout-max} {@code BatchWriteItem} chunks (default 25, DynamoDB hard cap 25).
 * Both writes are conditional and idempotent so a retry converges
 * without duplicates. A crash between those transactions leaves {@code lifecycleState} {@code CREATING}.
 * Snapshot reads then fail as not ready. A matching {@code clientRequestId} retry repairs missing
 * members and activates the conversation. Without that retry the conversation stays {@code CREATING}.
 *
 * <p>When {@code clientRequestId} is supplied, the service derives a stable conversation id and
 * stores a content fingerprint on the metadata row. A matching retry returns that conversation and
 * repairs missing member and inbox rows. A retry that reuses the id with different content is
 * rejected.
 *
...
public class ConversationService {
    ...
    public static final int DEFAULT_PARTICIPANT_MIN = 3;

    public static final int DEFAULT_PARTICIPANT_MAX = 256;
...
private final int participantMax;
    ...
    public ConversationService(ConversationRepository conversationRepository,
                               ...
                               InboxFanoutMax inboxFanoutMax,
                               @Value("${dynamodb.conversation-participant-min:" + DEFAULT_PARTICIPANT_MIN + "}")
                               int participantMin,
                               @Value("${dynamodb.conversation-participant-max:" + DEFAULT_PARTICIPANT_MAX + "}")
                               int participantMax) {
        this.conversationRepository = conversationRepository;
        this.userGraphRepository = userGraphRepository;
        this.conversationMapper = conversationMapper;
        this.inboxFanoutMax = inboxFanoutMax.value();
     ...
     * @throws ValidationException              when the creator is absent from the member set, a
     *     direct message carries a title, or {@code clientRequestId} is reused with different content
     */
    public CompletableFuture<CreateConversationResponse> createConversation(
        ...
        boolean replayable = isReplayable(request.clientRequestId());
        ...
        return requireAllProfiles(participants)
                .thenCompose(ignored -> replayOrWriteConversation(conversationId, type, request.title(),
                        participants, createdAt, replayable, requestFingerprint));
    }
    ...
    private String conversationId(String clientRequestId) {
        if (!isReplayable(clientRequestId)) {
            return "conv_" + IdGenerator.timeOrderedId();
        }
        return "conv_" + RequestIdentity.stableId("conversation", clientRequestId);
    }

    /** Treats a missing or blank identifier as a first attempt rather than a replay key. */
    private static boolean isReplayable(String clientRequestId) {
        return clientRequestId != null && !clientRequestId.isBlank();
    }
