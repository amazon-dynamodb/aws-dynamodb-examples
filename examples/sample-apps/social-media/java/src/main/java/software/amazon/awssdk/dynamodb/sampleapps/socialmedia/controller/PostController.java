...
private final PostContextService postContextService;
    ...
    @Operation(
            ...
            description = """
                    ...
                    RESTRICTED, none for PRIVATE). The default dynamodb.timeline-fanout-mode is SYNC, which \
                    writes those rows during this request. ASYNC returns after the source writes. In ASYNC \
                    mode timeline visibility may take at least dynamodb.async-minimum-visible-delay while \
                    DynamoDbStreamConsumer materializes the same rows. Media attachments are validated \
                    against the staged S3 objects and returned with presigned download URLs. Supplying \
                    clientRequestId lets callers replay the same publication and repair its timeline \
                    projections.""")
    @ApiResponse(responseCode = "201", description = "Post published",
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = PublishPostResponse.class)))
    @ApiResponse(responseCode = "400", description = "Missing X-User-Id, invalid visibility, invalid media, or reused clientRequestId with different content",
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
