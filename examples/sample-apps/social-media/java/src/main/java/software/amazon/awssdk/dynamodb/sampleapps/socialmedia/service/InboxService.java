 ...
 * the {@code limit}, and continuation-token depth first, then confirm the inbox owner has a profile
 ...
 * is route-specific, type-mode specific, and bound to the path user: a token minted for the timeline
 * route, the other type mode, or another user is rejected downstream. A token whose encoded depth
 * is at or above {@code dynamodb.max-pages-allowed} is rejected with
 * {@link InvalidPaginationTokenException}. DynamoDB I/O stays async end-to-end.
 */
@Service
...
private final MaxPagesAllowed maxPagesAllowed;
     ...
     * @param maxPagesAllowed        accepted {@code dynamodb.max-pages-allowed} cap
     */
     ...
     * @throws InvalidPaginationTokenException when {@code nextToken} is malformed or its depth is at
     *                                         or above {@code dynamodb.max-pages-allowed}
     */
    ...
    /**
     * Rejects a continuation token whose encoded depth is at or above the configured cap.
     *
     * @param nextToken opaque continuation, or {@code null} for the first page
     * @throws InvalidPaginationTokenException when the token is malformed or at the cap
     */
    private void rejectIfDepthAtCap(String nextToken) {
        if (nextToken == null || nextToken.isBlank()) {
            return;
        }
        if (PaginationTokenCodec.depth(nextToken) >= maxPagesAllowed.value()) {
