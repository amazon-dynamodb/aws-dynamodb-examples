 ...
 * identical item shape. The follow write is built by {@link FollowTransaction}.
 */
@Repository
...
public class LowLevelDynamoDbUserGraphRepository implements UserGraphRepository {
    ...
    public CompletableFuture<Void> followTransaction(FollowingEdge following, FollowerEdge follower) {
        return FollowTransaction.put(client, tableName, following, follower);
    }
    ...
    /**
     * Reads follower pages up to {@code dynamodb.follower-fanout-cap} after a rate-limit permit is
     * taken.
     *
