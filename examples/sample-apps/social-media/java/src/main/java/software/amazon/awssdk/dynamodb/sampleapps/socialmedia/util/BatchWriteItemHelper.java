 ...
 * call completed. Callers must resubmit the remaining write requests after a delay.
 * {@link ExponentialBackoffRetry#UNPROCESSED_ITEMS} bounds those resubmits. This helper is used by
 * the timeline fan-out and the inbox fan-out so no eligible recipient is silently skipped.
 */
public final class BatchWriteItemHelper {

    private static final RetryStrategy UNPROCESSED_ITEM_RETRY = ExponentialBackoffRetry.UNPROCESSED_ITEMS;
    ...
    public static CompletableFuture<Void> writeAllWithRetry(
            ...
            Logger logger) {
        return writeAllWithRetry(requestItems, 0, batchCallFn, logger, UNPROCESSED_ITEM_RETRY);
    }
     ...
     * @param retryStrategy unprocessed-item retry budget and backoff
    ...
    private static CompletableFuture<Void> writeAllWithRetry(
        ...
        return batchCallFn.apply(request).thenCompose(response -> {
            ...
            return delayAsync(retryStrategy.delay(attempt))
                    .thenCompose(ignored -> writeAllWithRetry(
                            unprocessed, attempt + 1, batchCallFn, logger, retryStrategy));
        });
    }
