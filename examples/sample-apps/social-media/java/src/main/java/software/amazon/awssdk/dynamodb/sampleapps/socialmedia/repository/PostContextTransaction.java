...
public final class PostContextTransaction {
    ...
    /**
     * Loads the post-context snapshot through the shared transaction.
     *
     ...
     */
    static CompletableFuture<PostContextItems> load(DynamoDbAsyncClient client, String contentTable,
                                                    String userGraphTable, String postId, String viewerId,
                                                    String authorId) {
        return client.transactGetItems(build(contentTable, userGraphTable, postId, viewerId, authorId))
                .thenApply(response -> toItems(response, viewerId, authorId));
    }
