...
class LowLevelDynamoDbUserGraphRepositoryFollowerQueryTest {
    ...
    @BeforeEach
    void setUp() {
        repository = new LowLevelDynamoDbUserGraphRepository(
                client, TABLE, new FollowerFanoutCap(2), FollowerEnumerationLimiter.withClock(1_000, System::nanoTime));
    }
    ...
    @Test
    void queryFollowers_whenRateLimited_skipsQueryAndReturnsIncomplete() {
        FollowerEnumerationLimiter limiter = FollowerEnumerationLimiter.withClock(1, () -> 0L);
        limiter.tryAcquire();
        repository = new LowLevelDynamoDbUserGraphRepository(client, TABLE, new FollowerFanoutCap(2), limiter);
