...
static final GenericContainer<?> DYNAMODB =
    ...
    @Test
    void readTimeline_whenTokenMintedForAnotherUser_returns400InvalidPaginationToken() {
        String author = createUser();
        String owner = createUser();
        String otherUser = createUser();
        follow(owner, author);
        publish(author, FIRST_POST_TEXT);
        publish(author, SECOND_POST_TEXT);
        publish(author, THIRD_POST_TEXT);

        ResponseEntity<PageBody> firstPage = readTimeline(owner, "?limit=2", PageBody.class);
        assertThat(firstPage.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(firstPage.getBody().nextToken()).isNotBlank();

        ResponseEntity<ErrorBody> response = readTimeline(otherUser,
